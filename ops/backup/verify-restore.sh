...
# ---------------------------------------------------------------------------
#  Внешнее хранилище. Локальная копия защищает от ошибки в данных, внешняя —
#  от потери сервера, и это разные риски: проверять нужно обе.
#
#  Сверяем существование и размер. Контрольную сумму взять не всегда можно:
#  S3 отдаёт SHA-256 не для всякой загрузки (у составных объектов его просто
#  нет), поэтому отсутствие суммы — не повод считать копию битой.
# ---------------------------------------------------------------------------
if [ -n "${S3_REMOTE:-}" ] && [ -n "${S3_BUCKET:-}" ]; then
    remote_path="${S3_REMOTE}:${S3_BUCKET}/${S3_PREFIX:-notescout}"
    set_name=$(basename "$(dirname "$latest")")
    remote_dir="${remote_path}/${set_name}"

    log "Проверяю внешнюю копию: ${remote_dir}"

    for suffix in "" ".sha256" ".counts"; do
        name="$(basename "$latest")${suffix}"
        local_size=$(wc -c < "${latest}${suffix}")

        remote_size=$(rclone size "${remote_dir}/${name}" --json 2>/dev/null \
            | sed -n 's/.*"bytes":\([0-9]*\).*/\1/p')

        if [ -z "$remote_size" ]; then
            fail "во внешнем хранилище нет ${name}: выгрузка не работает, копия только на этом сервере"
        fi
        [ "$remote_size" = "$local_size" ] \
            || fail "${name}: локально ${local_size} байт, в хранилище ${remote_size}"
    done

    log "  внешняя копия на месте, размеры совпадают"
else
    log "  внешнее хранилище не настроено (S3_REMOTE пуст) — проверена только локальная копия"
fi
