...
### Разовый chown тома вложений (обновление до версии с вложениями)

Если вы обновляетесь с версии **до появления вложений**, том `attachments`
создастся от `root`, а приложение работает под непривилегированным `app`:
загрузка вложения упадёт с `500`, хотя на старте всё выглядит исправно.

Начиная с версии с вложениями каталог создаётся в образе от имени `app`, и
**новый** том наследует эти права. Уже существующий том Docker так не
исправляет — нужен один разовый `chown`:

```bash
cd /opt/notescout
docker compose stop api backup
docker run --rm -u 0 -v notescout_attachments:/data \
  --entrypoint sh notescout/api:latest -c 'chown -R app:app /data'
docker compose up -d
```

`-u 0` обязателен: в образе по умолчанию пользователь `app`, а он менять
владельца не может. Проверка — `ls -ld` должен показать `app app`.
...
| `500` при загрузке вложения | том `attachments` принадлежит `root`, приложение пишет под `app` | разовый `chown` тома, см. раздел «Обновление версии» |
